                        ...
                        @if (auth()->user())
                            @if (Route::is('profile.index'))
                                <button id="dropdownNotificationButton" data-dropdown-toggle="dropdownNotification"
                                    ...
                                    @if (auth()->user()->receivedNotifications()->where('is_read', 0)->count() > 0)
                                        <div
                                ...
                                <div id="dropdownNotification"
                                    ...
                                    <div class="divide-y divide-gray-100 dark:divide-gray-700 overflow-y-auto"
                                        style="max-height: 400px;">
                                        @forelse (auth()->user()->receivedNotifications->sortByDesc('created_at') as $notification)
                                            <form action="{{ route('notification.update', $notification->id) }}"
                                                method="POST" class="w-100">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit"
                                                    class="btn w-100 text-left flex px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700">
                                                    ...
                                                    <div class="w-full ps-3">
                                                        <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">
                                                            <span
                                                                class="font-semibold d-block text-gray-900 dark:text-white d-flex items-center">
                                                                from {{ $notification->sender->name }}
                                                                @if ($notification->is_read == 0)
                                                                    <span
                                                                        class="d-block bg-red-500 rounded-circle ml-2"
